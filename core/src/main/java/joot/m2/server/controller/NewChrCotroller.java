...
public class NewChrCotroller extends Controller {
	...
	protected void onMessage(Message msg, ControllerContex ctx) throws IOException {
		...
		try (var redis = redisPool.getResource()) {
			...
			try {
				// TODO 校验数据
				trans.sadd("chrns", newChrReq.name);
				if (!chr1Exist)
					trans.hset("user:" + ses.una, "chr1", newChrReq.name);
				else
					trans.hset("user:" + ses.una, "chr2", newChrReq.name);
				...
				trans.hset("chr:" + newChrReq.name, chrInfo);
			} catch (Exception ex) {
				trans.discard();
				ex.printStackTrace();
			}
